...
export function registerOperations({ route, store, requireUser, guardAdmin, billingFor, stripe, request }) {
  ...
  route('post', '/api/billing/refund-requests', async (req, res) => {
    ...
    if (!stripe || !billing?.stripe_customer_id) return res.status(409).json({ error: 'No billed subscription was found.' });
    ...
    if (customer !== billing.stripe_customer_id) return res.status(404).json({ error: 'Invoice not found.' });
    const subId = invoice.subscription || invoice.parent?.subscription_details?.subscription;
    if (!subId || String(subId) !== String(billing.stripe_subscription_id) || invoice.status !== 'paid' || invoice.amount_paid <= 0) return res.status(400).json({ error: 'This invoice is not a paid marketplace subscription.' });
    const line = invoice.lines?.data?.find(item => item.period && (item.type === 'subscription' || item.parent?.type === 'subscription_item_details'));
    if (!line?.period) return res.status(409).json({ error: 'Contact support to review this invoice period.' });
  ...
  route('post', '/api/admin/refund/:id/review', async (req, res) => {
    ...
    const status = req.body?.status;
    if (!row || row.status === 'refunded' || !['approved', 'rejected'].includes(status)) return res.status(400).json({ error: 'Invalid refund review.' });
    const note = String(req.body?.note || '').trim();
