...
- Local test suite: 20 passing tests, including owner/provider completion flow, privacy, matching, duplicate repair submission, billing access, operational record isolation, refund review/idempotency, sanitization, and navigation behavior.
...
4. Test Stripe checkout and signed webhook delivery in **test mode**, including payment failure, subscription cancellation, repeat checkout, refund review, pending refund, and refund failure. No payment or refund was executed during this work. Confirm prices are USD 99/month and configure the customer portal. Refund input currently requires a Stripe invoice ID; a customer-facing invoice picker is a follow-up usability improvement.
...
- Never rerun the old `002_enable_rls.sql` after the new migration: its historical direct-client policies are superseded by backend-only access.
